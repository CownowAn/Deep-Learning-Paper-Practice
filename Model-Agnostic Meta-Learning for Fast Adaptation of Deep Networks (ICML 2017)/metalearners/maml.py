...
class ModelAgnosticMetaLearning(object):
    def __init__(self, model, optimizer=None, step_size=0.1, first_order=False,
        ...
        if per_param_step_size:
            self.step_size = OrderedDict((name, torch.tensor(step_size, dtype=param.dtype, device=self.device,
                                                             requires_grad=learn_step_size)) for (name, param) in model.meta_named_parameters())
        else:
    ...
    def get_outer_loss(self, batch):
        ...
        results = {
            ...
            'outer_losses': np.zeros((num_tasks, ), dtyep=np.float32),
            'mean_outer_loss': 0.
        }
        ...
        for task_id, (train_inputs, train_targets, test_inputs, test_targets) in enumerate(zip(*batch['train'], *batch['test'])):
            params, adaptation_results = self.adapt(train_inputs, train_targets, is_classification_task=is_classification_task,
                                                    num_adaptation_steps=self.num_adaptation_steps, step_size=self.step_size, first_order=self.first_order)
    ...
    def adapt(self, inputs, targets, is_classification_task=None, num_adaptation_steps=1, step_size=0.1, first_order=False):
        ...
        for step in range(num_adaptation_steps):
            ...
            params = gradient_update_parameters(self.model, inner_loss, step_size=step_size,
                                                params=params, first_order=(not self.model.training) or first_order)
    ...
    def train_iter(self, dataloader, max_batches=500):
        if self.optimizer is None:
            raise RuntimeError('Trying to call `train_iter`, while the optimizer is `None`.'
                               'In order to train `{0}`, you must specify a Pytorch oprimizer'
                               'as the argument of `{0}`'
                               '(eg. `{0}(model, optimizer=torch.optim.SGD(model.parameters(), lr=0.01), ...'
